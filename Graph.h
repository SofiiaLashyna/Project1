...
public:
    ...
    // --- Знаходження індексу вершини за id ---
    ...
    void addEdge(int fId, int tId, int weight = 1) {
        int f = findIndexById(fId);
        int t = findIndexById(tId);
        if (f == -1 || t == -1) return;
        if (edgeExists(fId, tId)) return;
        ...
        adjacencyMatrix[f][t] = 1;
        adjacencyMatrix[t][f] = 1;
    }
    ...
    void removeEdge(int fId, int tId) {
        int f = findIndexById(fId);
        int t = findIndexById(tId);
        if (f == -1 || t == -1) return;
    ...
    bool isConnectedList() {
        ...
        int start = -1;
        for (int i = 0; i < n; ++i) {
            if (!adjacencyList[i].empty()) { start = i; break; }
        }
        ...
        for (int i = 0; i < n; ++i) {
            if (!adjacencyList[i].empty() && !visited[i]) return false;
        }
        return true;
    }
        std::string vertexSearch(int id) {
...
private:
    void dfsList(int v, std::vector<bool>& visited) {
        visited[v] = true;
        for (auto& [neighbor, w] : adjacencyList[v])
            if (!visited[neighbor]) dfsList(neighbor, visited);
    }

    bool edgeExists(int fId, int tId) {
        int f = findIndexById(fId);
        int t = findIndexById(tId);
